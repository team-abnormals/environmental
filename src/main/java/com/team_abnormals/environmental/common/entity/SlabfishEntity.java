	...
	public void livingTick() {
		...
		for(PlayerEntity player : playerList) {
			if (player instanceof ServerPlayerEntity) {
				ServerPlayerEntity serverplayerentity = (ServerPlayerEntity) player;
				if(!world.isRemote()) {
					if (this.getSlabfishType() == SlabfishType.SWAMP) EnvironmentalCriteriaTriggers.SWAMP_SLABFISH.trigger(serverplayerentity); 
					if (this.getSlabfishType() == SlabfishType.MARSH) EnvironmentalCriteriaTriggers.MARSH_SLABFISH.trigger(serverplayerentity); 
				}
			}
		}
