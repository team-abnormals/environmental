...
public class SlabfishInBlockCondition implements SlabfishCondition {
	...
	public static SlabfishCondition deserialize(JsonObject json, JsonDeserializationContext context) {
		...
		return new SlabfishInBlockCondition(json.has("tag") ? null : ForgeRegistries.BLOCKS.getValue(new ResourceLocation(json.get("block").getAsString())), json.has("tag") ? TagCollectionManager.getInstance().getBlocks().getTag(new ResourceLocation(json.get("tag").getAsString())) : null);
	}
