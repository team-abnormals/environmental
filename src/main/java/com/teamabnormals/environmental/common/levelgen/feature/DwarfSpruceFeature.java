		...
		if (!this.initialized) {
			synchronized (this) {
				if (!this.initialized) {
					...
					this.heightNoise = NormalNoise.create(WorldgenRandom.Algorithm.LEGACY.newInstance(context.level().getSeed()).forkPositional().fromHashOf(EnvironmentalNoiseParameters.DWARF_SPRUCE_HEIGHT.location()), noise.getOrThrow(EnvironmentalNoiseParameters.DWARF_SPRUCE_HEIGHT));
